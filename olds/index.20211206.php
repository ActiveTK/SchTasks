        ...
        foreach($decs as $line) {
          ...
          if ($lined[0] == "::")
          {
            ...
            if ($lined[1] == "System.File")
            {
              ...
              else if (filesize(root.  "blob/" . $lined[2] . "/data") > 10485760)
              {
                echo "(ファイルサイズが10MBを超えているため、非表示になっています。<a href='?upload&watch=" . $lined[2] . "' target='_blank'>プレビュー</a>)";
                continue;
              }
              ...
              <?php
                ...
                echo "<iframe style='width:604px;height:340px;' sandbox src='?upload&watch=" . $lined[2] . "'></iframe>";
  ...
  <body style="background-color:#6495ed;color:#080808;overflow-x:hidden;overflow-y:visible;">
    ...
    if(isset($_FILES['uploadfile']) && is_uploaded_file($_FILES['uploadfile']['tmp_name'])) {
      ...
      if (file_exists(root . "blob/{$hashs}"))
        die(json_encode(array("error"=>"エラー: このファイルは既にアップロードされています。")));
      ...
      if (move_uploaded_file($_FILES['uploadfile']['tmp_name'], root . "blob/{$hashs}/data_"))
      {
        ...
        file_put_contents(root . "blob/{$hashs}/data",
          "ATKFMENCRYPTFILE!" . openssl_encrypt($dat, 'aes-256-cbc', password_md5, OPENSSL_RAW_DATA, '4910857128499038'));
...
<html lang="ja" itemscope="" itemtype="http://schema.org/WebPage" dir="ltr">
  ...
  <p>AES暗号(鍵256bit、CBCモード)で暗号化済み</p>
  <p><?=date("M d, Y (Y/m/d D)", strtotime(file_get_contents(root . "blob/{$pt}/date") . " 00:00:00"))?> - <?=convert(filesize(root . "blob/{$pt}/data"))?></p>
  <hr>
...
<head>
  ...
  <noscript><div title="NO SCRIPT ERROR" style="background-color:#404ff0;" align="center"><font color="#ff4500"><h1>No JavaScript Error.</h1></font></div></noscript>
    <div align="center" id="home">
      ...
      <p>アップロードされたファイルは、自動的に暗号化されます。</p>
